...
def get_message() -> str:
    """Gets the message to be sent in the subtask response."""
    # TODO: Implementar busca automática da mensagem no endpoint futuramente
    """
    url = f'{GLOBAL_SECURITI_URL}/privaci/v1/user/message_templates/category/dsr-messages/{data_subtask["message_collection"]}'
    default_message = "Esquecimento negado."
    try:
        response = requests.get(
            url,
            headers=data_dsr["secrets_header"],
            timeout=TIMEOUT,
            params={"enabled": "true", "published": "true"},
        )
    except requests.exceptions.Timeout:
        log_event("error", "get_message", "timeout", "Timeout")
        return default_message
    except requests.exceptions.RequestException as err:
        log_event("error", "get_message", "exception", str(err))
        return default_message

    message_id = data_subtask["message_id"]
    messages = response.json().get("message", [])
    for msg in messages:
        if msg.get("id") in {message_id}:
            return msg.get("body", default_message)
    """
...
def publish_dsr() -> bool:
    ...
    try:
        r = requests.post(
            ...
            headers=data_dsr["secrets_header"],
            json={
                "publish_subject": "Esquecimento Negado",
